...
import {appname, data, lsKey, ssKey, generateLSKey,setAdverts,weixinInstance} from './app'
//引入通用模板
$('body').append($(require('./commomTemplate.html')))

//customAjax基础方法
function baseCustomAjax(params){
  let url = config.interfaceUrls[0] + '/';
    return customAjax.get(url, params);
}
baseCustomAjax({}).done(function(result){}).fail(function(){})
...
//customAjax通用方法
customAjax.get(config.interfaceUrls[0] + '/', {}).done(function(result) {}).fail(function(){})

//微信分享
weixinInstance.config({ //基于http://gitlab.ewaytec.cn/cust-wicsg-frontend-team/weixin.git
    jsApiList: ['onMenuShareTimeline', 'onMenuShareAppMessage'],
    shareUrl: window.location.href.replace(/#/, '?'),
    shareTitle: result.title+'活动太好玩了，赶快来参加吧！',
    shareIconUrl: result.logoUrl,
    needOAuth2Url:true
})
